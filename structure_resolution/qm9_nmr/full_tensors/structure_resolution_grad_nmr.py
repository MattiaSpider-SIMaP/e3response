...
multistart with the neighbour list rebuilt at every step (so the topology stays
correct as the atom moves). This replaces the old `jax.scipy.optimize` BFGS,
whose strong-Wolfe line search failed on the float32-noisy loss (status=3, never
converging). The only thing this script supplies that ties the engine to the
full-tensor experiment is `loss_fn(pred_tensors) -> scalar` = the masked NMR-tensor
RMSE over atom k + its neighbours.
...
# topology follows the atom as it moves.
...
STARTS_PER_BATCH = 16    # vmap lanes in flight at once (bounds GPU memory)
...
# Live neighbour list: rebuild the graph's neighbours at every Adam step so the topology
# stays correct as the atom moves (a frozen list goes stale — see the module docstring).
...
def optimize_position(module, params, to_graph, atoms_base, atom_k,
    ...
    the topology follows the atom. Selection sees the LOSS alone (no ground truth).
    ...
    # One rebuilder per structure (captures the slot capacity, fixed for the run). Size
    # the capacity against a spread of the starts so it covers where the atom will roam.
    rebuild = None
    if LIVE_NEIGHBOURS:
    ...
    # Observable degeneracy check, plus a ground-truth diagnostic kept apart from it.
